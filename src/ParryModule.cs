...
public unsafe sealed partial class ParryModule : FhModule
{
    ...
    // MsEtEffectStop / MsEtEffectSet — effect-batch free / init. Call as a pair (Stop then
    // Set) to clear all active hit effects and re-arm the batch; Stop alone crashes the next fire.
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate void MsEtEffectStopProbe();
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate void MsEtEffectSetProbe();
    ...
    private void on_pre_update(Fahrenheit.Events.UpdateLoopEventArgs e)
    {
        ...
        // Debug FX/Motion Lab: auto-clear the hit-effect preview a short time after firing.
        tick_fx_motion_lab();
