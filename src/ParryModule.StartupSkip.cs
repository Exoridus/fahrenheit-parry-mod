...
// / fahrenheit-nolauncher mods). Those mods are alpha10 builds and no longer load
// against alpha11's ref-struct FhMethodHandle, so the behavior lives here for now,
// adapted to the ParryHook adapter. It is self-contained in this partial and always
// active (no parry setting gates it) — extract back into separate mods, or wire a
// setting, when convenient.
//
...
public unsafe sealed partial class ParryModule
{
    ...
    // ── hooks + state ────────────────────────────────────────────────────────
    private readonly ParryHook<StartupAtelEventSetUp>    _hStartupAtelEventSetUp;
    private readonly ParryHook<StartupNeedShowJapanLogo> _hStartupNeedShowJapanLogo;
    private readonly ParryHook<StartupFmvSkipPoll>       _hStartupBootFmvSkip;
    private readonly ParryHook<StartupShellExecuteW>     _hStartupShellExecuteW;
    ...
    private void tick_startup_skip()
    {
        ...
        try
        {
            _hStartupAtelEventSetUp.orig.Invoke(StartupSkipTitleRoomId);
            redirected = true;
        }
        catch (Exception ex)
        {
    ...
    private void h_startup_event_setup(uint eventId)
    {
        ...
        {
            _hStartupAtelEventSetUp.orig.Invoke(eventId);
            return;
        }
        ...
        _hStartupAtelEventSetUp.orig.Invoke(targetEventId);
    }
    ...
    private int h_startup_need_show_japan_logo()
    {
        ...
        return _hStartupNeedShowJapanLogo.orig.Invoke();
    }
    ...
    private void h_startup_boot_fmv_skip(nint fmv)
    {
        ...
        {
            _hStartupBootFmvSkip.orig.Invoke(fmv);
            return;
        }
        ...
        if (gMoviePlay == null || *gMoviePlay != 1) { _hStartupBootFmvSkip.orig.Invoke(fmv); return; }
        if (p[0x6d0] == 0 || p[0x6d2] == 0)         { _hStartupBootFmvSkip.orig.Invoke(fmv); return; }
    ...
    private IntPtr h_startup_shell_execute_w(IntPtr hwnd, string? lpOperation, string? lpFile, string? lpParameters, string? lpDirectory, int nShowCmd)
    {
        ...
        return _hStartupShellExecuteW.orig.Invoke(hwnd, lpOperation, lpFile, lpParameters, lpDirectory, nShowCmd);
    }
