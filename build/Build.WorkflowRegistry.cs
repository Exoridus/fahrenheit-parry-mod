...
internal sealed partial class BuildScript
{
    ...
    IReadOnlyList<WorkflowDefinition> CreateBuildWorkflowDefinitions()
    {
        ...
        [
            ...
            new("clean", "Core", "Remove local caches/artifacts (safe default)", "Default clean removes cache + build artifacts. Add explicit flags for analysis/exports/game-data/tools or use --purge for full local cleanup.", ["--analysis (optional).", "--exports (optional).", "--game-data (optional).", "--tools (optional).", "--purge (optional).", "--dry-run (optional)."], ["build.cmd clean", "build.cmd clean --analysis", "build.cmd clean --exports --game-data", "build.cmd clean --purge"], RunCleanCore),
