...
internal sealed partial class BuildScript
{
    ...
    IReadOnlyList<WorkflowDefinition> CreateToolsWorkflowDefinitions()
    {
        return
        [
            new("discord-sync", "Tooling Workflows (local-only)", "Export Discord JSON/Markdown + media + enrichment", "Export Discord channels/threads into .workspace/discord.", ["--guild <serverId> (required).", "--channels <id1,id2,...> (optional).", "--full (optional).", "Workspace config uses strict PascalCase keys in .workspace/config.local.json (DiscordToken, VisionApiUrl, VisionApiKey, VisionModel, FetchRetries).", "Discord workflow config uses strict PascalCase keys in .workspace/discord/config.local.json (Blacklist[], Guilds[])."], ["tools.cmd discord-sync --guild 612363389003366405"], DiscordSyncCore),
            ...
            new("data-parse", "Tooling Workflows (local-only)", "Run one parser mode", "Run one parser mode and capture output as txt.", ["--data-mode <MODE> (optional).", "--data-args \"<arg1> <arg2>\" (optional).", "--input-dir <path> (optional).", "--out-dir <path> (optional)."], ["tools.cmd data-parse --data-mode READ_ALL_COMMANDS"], () =>
            {
                CheckVbfExtractorReady();
                CheckDataParserReady();
                ParseDataCore();
            }),
            new("data-parse-all", "Tooling Workflows (local-only)", "Run parser mode batch", "Run configured parser mode batch and capture all outputs.", ["--data-batch \"MODE1;MODE2 arg\" (optional).", "--input-dir <path> (optional).", "--out-dir <path> (optional)."], ["tools.cmd data-parse-all"], () =>
            {
                CheckVbfExtractorReady();
                CheckDataParserReady();
                ParseDataAllCore();
