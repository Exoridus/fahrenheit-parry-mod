...
internal sealed partial class BuildScript : NukeBuild
{
    ...
    void ShowHelpSummary()
    {
        ...
        WriteHelpLine(string.Empty);
        WriteHelpLine("  tools.cmd    Local-only tooling workflows (data/discord/ghidra moved to ffx-knowledge-base)");
    ...
    void ReleaseVersionCore()
    {
        ...
        var filesToStage = new[]
        {
            ...
            Quote(ManifestPath),
            Quote(RootDirectory / "Fahrenheit.Mods.Parry.csproj"),
    ...
    void UpdateManifestVersion(SemVersion version, string repoSlug)
    {
        ...
        UpdateProjectVersion(version, repoSlug);
    }

    // The shipped manifest is generated by Fahrenheit.Sdk from assembly properties
    // (__FhEmitManifest), so the mod project carries the version the release
    // actually gets. fhparry.manifest.json stays the checked-in reference the
    // quality checks read; both are written here so they cannot drift.
    void UpdateProjectVersion(SemVersion version, string repoSlug)
    {
        var projectPath = RootDirectory / "Fahrenheit.Mods.Parry.csproj";
        if (!File.Exists(projectPath))
        {
            Fail($"Mod project not found: {projectPath}");
        }
        ...
        if (!string.IsNullOrWhiteSpace(repoSlug))
        {
            text = Regex.Replace(
                ...
                $"<RepositoryUrl>https://github.com/{repoSlug}</RepositoryUrl>");
        }
